...
if withMTLFile
 ...
 M.mtlFile.data = read_MTL_file([mtlFilePth '/' mtlFileName]);
        ...
        M.mtlFile.data = M.mtlFile.data(mmp(2)+1);
        try
...
for i=1:length(M.mtlFile.data)
    M.mtlFile.data(i).mtl.map_Ka
M.texMap.imgs{i} = imread([mtlFilePth,'/',M.mtlFile.data(i).mtl.map_Ka]);
    ...
    try
        ...
        M.texMap.img = imread([M.mtlFile.path,'/',M.mtlFile.data.mtl.map_Ka]);
    catch
        'no k map';
    end
end
